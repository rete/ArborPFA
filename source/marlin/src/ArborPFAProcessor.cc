/**
 *  @file   marlin/src/ArborPFAProcessor.cc
 * 
...
pandora::StatusCode ArborPFAProcessor::RegisterUserComponents() const
{
    ...
    //    "MyHadronicEnergyCorrection", pandora::HADRONIC, &ArborPFAProcessor::MyHadronicEnergyCorrection));

    //PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, PandoraApi::RegisterParticleIdFunction(*m_pPandora, "MyParticleId",
    //    &ArborPFAProcessor::MyParticleId));
...
void ArborPFAProcessor::ProcessSteeringFile()
{
   ...
   registerProcessorParameter("UseOldTrackStateCalculation",
    ...
    // flag to map sdhcal data (sim or TB) to hcal endcap
    registerProcessorParameter("ShouldUseSDHCALData",
                            "If we are dealing with SDHCAL data or not",
                            m_caloHitCreatorSettings.m_shouldUseSDHCALData,
                            true);
}
